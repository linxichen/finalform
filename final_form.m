...
while ((outer_diff > outer_tol) && (outer_iter < maxiter))
    ...
    while err > tol
        for i_s = 1:ns
            ...
            for i_K = 1:nK
                ...
                %=========================================================%
                for i_q = 1:nq
                    netprofit = repmat(profit,1,nk)-q_grid(i_q)*(inv_mat).*(pos_inv+neg_inv*pphi);
                    candidate = llambda*netprofit  + bbeta*repmat(EV_invest',nk,1);
                    %=====================================================%
    ...
    for t = 1:T
        ...
        for i_q = 1:nq
            tot_profit_grid = (q_grid(i_q)-w)*dist_k(:,:,t).*(kopt_fine(:,whichs,i_K,i_q)-(1-ddelta)*repmat(fine_grid,1,nx)).*(active_fine(:,whichs,i_K,i_q));
    ...
    outer_diff = norm([pphi_K,pphi_q,pphi_C]-[pphi_K_new,pphi_q_new,pphi_C_new],Inf);
...
%     plot(q_grid,demand(:,T),'-.r',q_grid,demand(:,T-7),'b');
    ...
    disp(disp_text);
    disp('===============================');
    save('aggrules.mat','pphi_K','pphi_q','pphi_C');
    save('Rsq.mat','Rsq_K','Rsq_q','Rsq_C');
